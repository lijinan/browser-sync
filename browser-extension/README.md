...
扩展使用统一的 Manifest V3 配置，自动适配 Chrome/Edge 和 Firefox，无需手动切换。
...
**临时安装**:
```bash
1. 打开 Firefox 浏览器 (版本 109+)
2. 访问 about:debugging
3. 点击"此Firefox"
4. 点击"临时载入附加组件"
5. 选择 browser-extension 文件夹中的 manifest.json 文件
```

**永久安装**:
```bash
1. 使用 web-ext 工具打包: web-ext build
2. 生成 .xpi 文件
3. 拖拽到 Firefox 安装
```

> **注意**: Firefox 需要版本 109 或更高版本才能支持 Manifest V3。
...
```
browser-extension/
├── manifest.json              # 统一的清单文件 (兼容 Chrome/Edge/Firefox)
...
**说明**: 
- 使用统一的 `manifest.json`，自动适配 Chrome/Edge 和 Firefox
- Chrome 和 Firefox 都使用 Manifest V3 格式
- 所有后台逻辑都在根目录，无需区分浏览器
